...
def generate_rss(atom=False):
    """Generates the RSS feed by scraping Bandcamp content."""
...
async def _rss_feed(atom=False):
    """Endpoint to generate and return the RSS feed, with caching."""
    ...
    # Generate new feed if cache is expired or not set
    try:
        ...
        logger.error(f"Failed to generate {feed_type} feed: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to generate feed")
    ...
    # return Response(content=rss_content, media_type=media_type, status_code=status.HTTP_200_OK)


@app.get("/rss", response_class=Response)
async def rss_feed():
    """Endpoint to generate and return the RSS feed."""
    return await _rss_feed(atom=False)


@app.get("/atom", response_class=Response)
async def atom_feed():
    """Endpoint to generate and return the RSS feed."""
    return await _rss_feed(atom=True)
